...
def run_webserver(host: str, port: int):
    ...
    sockets_to_read = {listening_socket: listening_socket.getsockname()} # <-------
    try:
        while True:
            readables, _, _ = select.select(sockets_to_read.keys(), [], [])  # <---------
            for readable_socket in readables:
                if readable_socket == listening_socket:
                    client, address = listening_socket.accept()
                    print(f"Connected to client {address}") # <---------
                    client.setblocking(False)
                    sockets_to_read[client] = address # <---------
                else:
                    close_connection = handle_connection(readable_socket, sockets_to_read[readable_socket]) # <-------
                    if close_connection:
